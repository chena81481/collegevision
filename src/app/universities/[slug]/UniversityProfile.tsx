...
export default function UniversityProfile({ initialData, competitors }: { initialData?: any, competitors?: any[] }) {
  ...
  const [university, setUniversity] = useState<University | null>(initialData || null);
  const [selectedCourse, setSelectedCourse] = useState<Course | null>(initialData?.courses?.[0] || null);
  ...
  // ── Fetch from Supabase (Only if initialData not provided) ───────────────────
  useEffect(() => {
    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-24 selection:bg-teal-200 selection:text-teal-900">
      ...
      <div className="max-w-7xl mx-auto px-6 mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* ── 3. Competitive Comparison Section ────────────────────────── */}
          {competitors && competitors.length > 0 && (
            <div className="bg-white rounded-3xl p-8 border border-slate-200 mt-12 shadow-sm">
                ...
                {competitors.map((comp) => {
                    const avgFee = comp.courses?.[0]?.total_fee_inr || 0;
                    const avgSalary = comp.courses?.[0]?.avg_ctc_inr || 0;
                    ...
                    <div key={comp.slug} className="bg-slate-50 p-6 rounded-2xl border border-slate-100 hover:border-blue-500/30 transition-all group">
                        ...
                        <div>
                            <div className="text-slate-400 font-bold text-[10px] uppercase tracking-wider">Avg CTC</div>
                            <div className="font-black text-slate-700">{formatINR(avgSalary)}</div>
                        </div>
                        <div>
                            <div className="text-slate-400 font-bold text-[10px] uppercase tracking-wider">Total Fee</div>
                            <div className="font-black text-slate-700">{formatINR(avgFee)}</div>
                        </div>
                        </div>
                        <Link 
